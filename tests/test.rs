use hmac::digest::generic_array::typenum::consts::U32;
...
fn test1_sha256() {
    ...
    assert_eq!(
        ...
        hex::decode("018ec5f8e08c41e5ac974eb129ac297c5388ee1864324fa13d9b15cf98d9a157")
            .unwrap()
            .as_slice()
    );
}
...
fn test2_sha256() {
    ...
    assert_eq!(
        drbg.generate::<U32>(None).as_slice(),
        hex::decode("ed5d61ecf0ef38258e62f03bbb49f19f2cd07ba5145a840d83b134d5963b3633")
            .unwrap()
            .as_slice()
    );
}
...
fn reseeding() {
    ...
    assert_eq!(
        original.generate::<U32>(None),
        reseeded.generate::<U32>(None)
    );
...
fn nist_victors() {
    ...
    let tests: Vec<Fixture> =
        serde_json::from_str(include_str!("./fixtures/hmac-drbg-nist.json")).unwrap();
    ...
    for test in tests {
        ...
        drbg.generate_to_slice(
            &mut result[0..full_len],
            match add0 {
